...
def check_expiring_domains():
    ...
    try:
        ...
        for domain, user_id, expiry_date in expiring_domains:
            ...
            days_until_expiry = (
                datetime.strptime(expiry_date, "%Y-%m-%d") - today
            ).days
        ...
        for user_id, domains in user_domains.items():
            ...
            try:
                ...
                logging.info(
                    f"Notification sent to user {user_id} for {len(domains)} domains"
                )
        ...
        logging.info(
            f"Expiry check completed. Processed {len(expiring_domains)} expiring domains"
        )
