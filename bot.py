...
def log_message(message):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            """INSERT INTO message_logs (user_id, message_text, command) 
                    VALUES (?, ?, ?)""",
...
def add_domain(message):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "SELECT * FROM domains WHERE domain=? AND user_id=? AND is_deleted=0",
...
def list_domains(message):
    ...
    c = conn.cursor()
    c.execute(
        "SELECT domain, expiry_date FROM domains WHERE user_id=? AND is_deleted=0",
...
def delete_domain(message):
    ...
    try:
        ...
        c = conn.cursor()
        c.execute(
            "UPDATE domains SET is_deleted=1 WHERE domain=? AND user_id=?",
            (domain, message.from_user.id),
        )
        conn.commit()
        if c.rowcount > 0:
...
def check_domains(message):
    ...
    if message.text.startswith("/check "):
        ...
        domains = [(domain,)]
        c.execute(
            "SELECT domain FROM domains WHERE domain=? AND user_id=? AND is_deleted=0",
            (domain, message.from_user.id),
        )
        if not c.fetchone():
        ...
        c.execute(
            "SELECT domain FROM domains WHERE user_id=? AND is_deleted=0",
            (message.from_user.id,),
        )
        domains = c.fetchall()
        if not domains:
